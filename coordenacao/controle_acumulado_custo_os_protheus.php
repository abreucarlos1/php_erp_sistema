...
/*
	...
	Versão 2 --> Atualização layout: 28/11/2014
...
function preencheos($id_coordenador)
{
	...
	{
		$comb .= "xajax.$('escolhaos').options[xajax.$('escolhaos').length] = new Option('". sprintf("%05d",$os["os"])." - ".substr($os["descricao"],0,50)." - ".$os["empresa"] ."','". $os["os"] ."');";
	}

	$resposta->addScript($comb);
	
	$resposta->addScript("displayByClass('trOs', 'show')");
...
$sql = "SELECT funcionarios.id_funcionario, funcionarios.funcionario FROM ".DATABASE.".ordem_servico, ".DATABASE.".ordem_servico_status, ".DATABASE.".funcionarios ";
...
$smarty->assign("campo",$conf->campos('controle_acumulado_custo_os_protheus'));

$smarty->assign("botao",$conf->botoes());

$smarty->assign("classe",CSS_FILE);

$smarty->display('controle_acumulado_custo_os_protheus.tpl');
?>