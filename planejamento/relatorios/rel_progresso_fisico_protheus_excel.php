...
//EXCESSOES PARA DESMEMBRAMENTO (atividades que n�o podem ser desmembradas)
...
//Percorre as tarefas da fase de or�amento
...
{	
	...
	switch ($disciplina)
	{
		...
		case 'MEC': //MEC�NICA
			$discip = 4;
			$array_disc[4] = 'MEC�NICA';
		break;
		
		case 'TUB': //TUBULA��O
			$discip = 5;
			$array_disc[5] = 'TUBULA��O';
		break;
		
		case 'ELE': //ELETRICA
			$discip = 6;
			$array_disc[6] = 'EL�TRICA';
		break;
		
		case 'INS': //INSTRUMENTA��O
			$discip = 7;
			$array_disc[7] = 'INSTRUMENTA��O';
		break;
		
		case 'AUT': //AUTOMA��O
			$discip = 8;
			$array_disc[8] = 'AUTOMA��O';
		break;
		
		case 'EST': //ESTRUTURA MET�LICA
			$discip = 9;
			$array_disc[9] = 'ESTRUTURA MET�LICA';
		break;
		...
		case 'SEG': //SEGURAN�A
			$discip = 11;
			$array_disc[11] = 'SEGURAN�A';
		break;

		case 'VAC': //VENTILA��O E AR CONDICIONADO
			$discip = 12;
			$array_disc[12] = 'VENTILA��O E AR CONDICIONADO';
		break;
	...
	//verifica as composi��es, para agregar na composi��o pai.
	switch($composicao)
	{
		/*
		case 'COR12': //COORDENA��O
			...
			$array_titulo[$discip]['GER20'] = 'MOBILIZA��O';
...
$objPHPExcel->getProperties()
            ->setCreator("DEVEMADA ENGENHARIA")
...
foreach($array_titulo as $indice=>$valor)
{
	...
	{		
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $row_cell, iconv('ISO-8859-1', 'UTF-8', trim($regs0["A1_NOME"]).'-'.trim($regs0["A1_MUN"])));
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row_cell, iconv('ISO-8859-1', 'UTF-8', trim($regs0["AF1_ORCAME"])));
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row_cell, iconv('ISO-8859-1', 'UTF-8', str_replace("/"," ",trim($regs0["AF1_DESCRI"]))));

		//Verifica se o formato � horas ou disciplina esta nas excess�es
		if($array_formato[$indice][$index]=='HR' || $array_formato[$indice][$index]=='A4' || in_array($array_compos[$indice][$index],$excessao))
		{			

			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row_cell, iconv('ISO-8859-1', 'UTF-8',$array_disc[$indice]));

			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row_cell, iconv('ISO-8859-1', 'UTF-8', $descricao_doc));
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_formato[$indice][$index]));
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_quant_docs[$indice][$index]));
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_grau[$indice][$index]));
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_quant_horas[$indice][$index]));
		...
		else
		{
			...
			for($i=1;$i<=($array_quant_docs[$indice][$index]);$i++)
			{
				...
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row_cell, iconv('ISO-8859-1', 'UTF-8', $descricao_doc."_".$i));
				
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_formato[$indice][$index]));
				
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row_cell, iconv('ISO-8859-1', 'UTF-8', 1));
				
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $row_cell, iconv('ISO-8859-1', 'UTF-8', $array_grau[$indice][$index]));				

				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row_cell, iconv('ISO-8859-1', 'UTF-8', ($array_quant_horas[$indice][$index]/$array_quant_docs[$indice][$index])));
