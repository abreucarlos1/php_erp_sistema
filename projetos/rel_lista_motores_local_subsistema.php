...
$sql3000 .= "WHERE locais.id_equipamento = equipamentos.id_equipamentos ";
...
$pdf->titulo = $reg3000["cd_local"] . " " . $reg3000["nr_sequencia"] . " " . $reg3000["ds_complemento"];
...
$sql2 .= "LEFT JOIN Projetos.locais ON (locais.id_local = componentes.id_local AND locais.id_local = '" . $_POST["id_local"] . "' )";
...
//$sql2 .= "WHERE malhas.id_subsistema = '".$reg["id_subsistema"]."' ";	
$sql2 .= "WHERE area.id_os = '" . $_SESSION["id_os"] . "' ";
...
$sql1 .= "WHERE area.id_os = '" . $_SESSION["id_os"] . "' ";
...
{
//	die(print_r($demarrador));
	if($demarrador[$malhas["nr_malha"]."#".$separador.$malhas["nr_malha_seq"]])
	{
		...
		{
			$nrmalha1 = '';
			$tam = strlen($malhas["nr_malha"]);
			for($y=0;$y<3-$tam;$y++)
			{
				$nrmalha1 = $nrmalha1 . '0';
			}
			$nrmalha1 = $nrmalha1 . $malhas["nr_malha"];
		}
		else
		{
		...
		$pdf->Cell(22,5,$espc["CORRENTE NOMINAL"],1,0,'C',0);
