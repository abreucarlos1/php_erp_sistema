...
$registro1 = mysql_query($sql1,$conexao) or die("Não foi possível a seleção dos dados" . $sql);
	...
	while ($malhas = mysqli_fetch_array($regmalha))
	{
		...
		if($db->numero_registros>0)
		{
			...
			*/
			...
			$pdf->SetFont('Arial','',8);
			$pdf->Cell(30,4,"DESCRIÇÃO",0,0,'L',0);
			$pdf->SetFont('Arial','B',8);
			...
			$pdf->SetFont('Arial','B',10);
			$pdf->Cell(20,4,"2 - ESPECIFICAÇÃO PADRÃO" ,0,1,'L',0);
			$pdf->Ln(2);				
