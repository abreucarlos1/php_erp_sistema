...
/*
		...
		Formul�rio de Racks - Cadastro de Racks	
...
if ($_GET["acao"]=="deletar")
{
	...
	<script>
		...
		alert('Rack exclu�do com sucesso.');
	</script>
	<?
}
...
switch ($_POST["acao"])
{
	...
	case 'editar':
		...
		if ($db->numero_registros>0)
		{
			...
			<script>
				...
				alert('Rack j� cadastrado no banco de dados.');
	...
	// Caso contrario, insere o campo com as variaveis 'postadas' pelo formulario
	else
		{
			...
			$incsql = "INSERT INTO Projetos.racks ";
			$incsql .= "(id_local, id_devices, nr_rack, cd_fabricante, nr_capacidade) VALUES (";
			$incsql .= "'". $_POST["id_local"] ."', ";
			$incsql .= "'". $_POST["id_devices"] ."', ";
			$incsql .= "'". maiusculas($_POST["nr_rack"]) ."', ";
			$incsql .= "'". maiusculas($_POST["cd_fabricante"]) ."', ";
			$incsql .= "'". maiusculas($_POST["nr_capacidade"]) ."') ";
    ...
    <tr>
      ...
      <td colspan="5"><table width="100%" border="0">
        <tr>
          ...
          <td width="1%">&nbsp;</td>
          <td width="13%"><span class="label1">N&ordm; RACK</span></td>
          <td width="1%">&nbsp;</td>
		...
		{
			?>
            <option value="<?= $cont_area["id_local"] ?>" <? if($rack["id_local"]==$cont_area["id_local"]) { echo "selected"; } ?>>
		...
		*/
			...
			{
			?>
			<option value="<?= $cont["id_local"] ?>" <? if($cont["id_local"]==$rack["id_local"]) { echo "selected"; } ?>>
		...
		$sql1 .= "WHERE OS.id_empresa_erp = empresas.id_empresa_erp ";
		...
		$sql_area .= "WHERE id_cliente = '" . $empresa["id_empresa_erp"] . "' ORDER BY cd_dispositivo";
		...
		while($cont_area = mysqli_fetch_array($reg_area))
		{
			...
			?>
            <option value="<?= $cont_area["id_devices"] ?>"<? if($rack["id_devices"]==$cont_area["id_devices"]){ echo 'selected';} ?>>
    ...
    <tr>
      ...
      <td colspan="5"><table width="100%" border="0">
        <tr>
          ...
          <td width="1%">&nbsp;</td>
          <td width="13%"><span class="label1">N&ordm; REVIS&Atilde;O</span></td>
          <td width="1%">&nbsp;</td>
          <td width="72%">&nbsp;</td>
        </tr>
        <tr>
    ...
    <tr>
      ...
      <td colspan="3"><table width="100%" border="0">
        <tr>
          ...
          <td width="1%">&nbsp;</td>
          <td width="18%"><span class="label1">N&ordm; RACK </span></td>
          <td width="1%">&nbsp;</td>
		...
		{
			?>
            <option value="<?= $cont_area["id_local"] ?>"<? if($cont_area["id_local"]==$_POST["id_local"]){ echo 'selected';} ?>>
              <?= $cont_area["nr_area"] . " - " . $cont_area["nr_local"] . " - " . $cont_area["nr_sequencia"] ?>
              </option>
            <?
		}
		*/
			...
			{
			?>
			<option value="<?= $cont["id_local"] ?>" <? if($cont["id_local"]==$_POST["id_local"]) { echo "selected"; } ?>>
			<?= $cont["nr_area"]. " - ".  $cont["cd_local"]. " ". $cont["nr_sequencia"]. " - ". $cont["ds_equipamento"] ?>
			</option>
			<?
		...
		while($cont_area = mysqli_fetch_array($reg_area))
		{
			...
			?>
            <option value="<?= $cont_area["id_devices"] ?>"<? if($cont_area["id_devices"]==$_POST["id_devices"]){ echo 'selected';} ?>>
            <?=  $emp . " - " .$cont_area["cd_dispositivo"] ?>
            </option>
            <?
		}
    ...
    <tr>
      ...
      <td colspan="3"><table width="100%" border="0">
        <tr>
          <td width="13%"><span class="label1">capacidade</span></td>
          <td width="1%">&nbsp;</td>
          <td width="86%"><span class="label1">N&ordm; REVIS&Atilde;O</span></td>
        </tr>
        <tr>
...
<table width="100%" class="cabecalho_tabela" cellpadding="0" cellspacing="0" border=0>
    <tr>
      ...
      <td width="26%" class="cabecalho_tabela">N� RACK</td>
...
<div id="tbbody" style="position:relative; width:100%; height:263px; z-index:2; overflow-y:scroll; overflow-x:hidden; border-color:#999999; border-style:solid; border-width:1px;" >
  <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" class="corpo_tabela">
	<?
		...
		$sql .= "AND devices.id_cliente = '" . $empresa["id_empresa_erp"] . "' ";
