...
*/
		...
		*/
		...
		$sql1 = "SELECT * FROM Projetos.malhas, Projetos.processo, Projetos.subsistema ";
		$sql1 .= "WHERE malhas.id_subsistema = subsistema.id_subsistema ";
		$sql1 .= "AND subsistema.id_area = '" .$_POST["id_area"] . "'";
		...
		while ($malhas = mysqli_fetch_array($regmalha))
		{
			...
			*/
			...
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(90,4,$malhas["processo"]." - ".$nrmalha."   -   ".$malhas["ds_servico"],0,1,'L',0);
			$pdf->SetFont('Arial','',8);
			...
			while ($componentes = mysqli_fetch_array($regcomp))
			{
				...
				$pdf->HCell(33,4,$reg["nr_area"] . " - " . $processo . $componentes["dispositivo"]. " - " . $nrmalha . $nrseq . $modificador ,0,0,'L',0);
