...
$pdf->SetXY(25,$y);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(50,4,'CONTROLE DE REVISÕES' . $z,0,1,'L',0);
$pdf->SetFont('Arial','',6);
...
while($revis = mysqli_fetch_array($reg_rev))
{
	...
	$contexe = mysqli_fetch_array($regexe);
	
	$executante = $contexe["abreviacao"];
	...
	$contver = mysqli_fetch_array($regver);
	
	$verificador = $contver["abreviacao"];
	...
	$contapr = mysqli_fetch_array($regapr);
	
	$aprovador = $contapr["abreviacao"];
...
while ($locais = mysqli_fetch_array($regis1))
{
...
$sql0 .= "AND area.id_area = '".$_POST["id_area"]."' ";

$regis0 = $db->select($sql0,'MYSQL');

while ($comp = mysqli_fetch_array($regis0))
{
	...
	$sql .= "AND cabos.id_cabo_tipo = cabos_tipos.id_cabo_tipo ";
	$sql .= "AND cabos.id_disciplina = '".$_POST["disciplina"]."' ";
	$sql .= "AND cabos.id_origem_comp = '".$comp["id_componente"]."' ";
	$sql .= "ORDER BY cabos.id_cabo, id_origem_local, ds_formacao, seq_veia ";
	...
	while ($cabos = mysqli_fetch_array($registro))
	{
		...
		$origem = $comp["nr_area"]." ".$processo . $comp["dispositivo"] . "  " .$nrmalha.$nrseq.$modificador ;
