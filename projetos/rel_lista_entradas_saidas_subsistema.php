...
function Header()
{
	...
	$this->Cell(66,5.5,'',0,0,'L',0); // C�LULA LOGOTIPO
	$this->SetFont('Arial','B',10);
	$this->Cell(140,5.5,"LISTA DE COMPONENTES",1,0,'C',0); // C�LULA COMPONENTE
...
if(mysql_num_rows($regmalha)>0)
{
	while ($malhas = mysql_fetch_array($regmalha))
	{
		...
		$regend = mysql_query($sql2,$db->conexao) or die("N�o foi poss�vel a sele��o dos dados" . $sql2);
		...
		while ($enderecos = mysql_fetch_array($regend))
		{
			...
			$regcom = mysql_query($sql3,$db->conexao) or die("N�o foi poss�vel a sele��o dos dados" . $sql3);
