...
function Header()
{
	...
	*/
	...
	$this->Cell(45,5.5,'',0,0,'L',0); // CÉLULA LOGOTIPO
	$this->SetFont('Arial','B',10);
	$this->Cell(85,5.5,"ESPECIFICAÇÃO TÉCNICA / MALHAS",1,0,'C',0); // CÉLULA COMPONENTE
...
$sql .= "WHERE malhas.id_malha = '" .$_POST["id_malha"]. "' ";
...
$sql .= "WHERE malhas.id_malha = '" . $reg["id_malha"] . "' ";
$sql .= "AND subsistema.id_subsistema = malhas.id_subsistema ";
...
$sql = "SELECT * FROM Projetos.subsistema, Projetos.malhas, Projetos.processo, Projetos.funcao, Projetos.dispositivos, Projetos.tipo, Projetos.componentes ";
$sql .= "LEFT JOIN Projetos.locais ON (componentes.id_local = locais.id_local) ";
$sql .= "LEFT JOIN Projetos.equipamentos ON (locais.id_equipamento = equipamentos.id_equipamentos) ";
$sql .= "LEFT JOIN Projetos.especificacao_tecnica ON (componentes.id_componente = especificacao_tecnica.id_componente) ";
$sql .= "WHERE malhas.id_malha = '" . $reg["id_malha"] . "' ";
$sql .= "AND subsistema.id_subsistema = malhas.id_subsistema ";
$sql .= "AND malhas.id_malha = componentes.id_malha ";
$sql .= "AND malhas.id_processo = processo.id_processo ";
...
if($db->numero_registros>0)
{
	while ($malhas = mysqli_fetch_array($regmalha))
	{
		...
		$pdf->SetY(34);
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(45,5.5,"",0,0,'L',0); // CÉLULA LOGOTIPO
		$pdf->HCell(85,5.5,"TAG: ".$reg["nr_area"]." ".$processo."".$malhas["dispositivo"]." ".$malhas["nr_malha"]." ".$malhas["funcao"],1,0,'C',0); // CÉLULA COMPONENTE
		$pdf->SetX($posax);
		$pdf->SetY($posay);
		*/
		...
		if($db->numero_registros>0)
		{
			...
			*/
			...
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(50,5,$reg["ds_area"] ,0,1,'L',0);
