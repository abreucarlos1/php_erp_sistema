...
function Header()
{
	...
	*/
	...
	$this->Cell(45,5.5,'',0,0,'L',0); // C�LULA LOGOTIPO
	$this->SetFont('Arial','B',10);
	$this->Cell(85,5.5,"ESPECIFICA��O T�CNICA / MALHAS",1,0,'C',0); // C�LULA COMPONENTE
...
if($db->numero_registros>0)
{
	while ($malhas = mysqli_fetch_array($regmalha))
	{
		...
		$pdf->SetY(34);
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(45,5.5,"",0,0,'L',0); // C�LULA LOGOTIPO
		$pdf->HCell(85,5.5,"TAG: ".$reg["nr_area"]." ".$processo."".$malhas["dispositivo"]." ".$malhas["nr_malha"]." ".$malhas["funcao"],1,0,'C',0); // C�LULA COMPONENTE
		$pdf->SetX($posax);
		$pdf->SetY($posay);
		*/
