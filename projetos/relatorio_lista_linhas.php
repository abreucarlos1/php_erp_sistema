...
<html>
<head>
<title>: : . RELAT&Oacute;RIOS LISTA DE LINHAS . : :</title>
...
function enviar_area(area)
{
	...
	if((area!='') && (document.forms['areas'].emissao.checked))
	{
		...
		document.forms['areas'].relatorio.value='lista_linhas_area';
	...
	else
	{
		if(area!='')
		{
			if(document.forms['areas'].excel.checked)
			{
				//document.forms['areas'].acao.value='';
				//document.forms['areas'].relatorio.value='lista_linhas_area';
				//document.forms['areas'].action='excel_lista_linhas_area.php';
				//document.forms['areas'].submit();			
			}
			else
			{
				document.forms['areas'].acao.value='';
				document.forms['areas'].relatorio.value='lista_linhas_area';
				document.forms['areas'].action='rel_lista_linhas_area.php';
				document.forms['areas'].submit();
			}
		}
	}
...
function enviar_subsistema(area)
{
	...
	if((area!='') && (document.forms['areas'].emissao.checked))
	{
		document.forms['areas'].target='_self';
		document.forms['areas'].action='<?= $PHP_SELF ?>';
		document.forms['areas'].relatorio.value='lista_linhas_subsistema';
		document.forms['areas'].acao.value='salvar';
		document.forms['areas'].submit();
	...
	else
	{
		if(area!='')
		{
			...
			else
			{
				document.forms['areas'].acao.value='';
				document.forms['areas'].relatorio.value='lista_linhas_subsistema';
				document.forms['areas'].action='rel_lista_linhas_subsistema.php';
				document.forms['areas'].submit();
			}
		}
	}
      ...
      <tr>
        <td>
		  <table width="100%" height="100%" border="0">
            ...
            <tr>
              <td colspan="7" align="center" class="kks_nivel1"> lista de LINHAS </td>
              </tr>
            <tr class="btn">
              <td class="btn">&nbsp;</td>
...
<?
	...
	{
		$envio_rel = false;
		if($_POST["relatorio"]=='lista_linhas_area')
		{
			?>	
				<script>	
		...
		if($_POST["relatorio"]=='lista_linhas_subsistema')
		{
			?>	
				<script>	
