...
<html>
<head>
<title>: : . RELAT&Oacute;RIOS ESPECIFICA&Ccedil;&Atilde;O T&Eacute;CNICA . : :</title>
...
function enviar_area(area)
{
	...
	if((area!='') && (document.forms['areas'].emissao.checked))
	{
		...
		document.forms['areas'].relatorio.value='especifica��o_tecnica_area';
...
function enviar_subsistema(area)
{
	...
	if((area!='') && (document.forms['areas'].emissao.checked))
	{
		document.forms['areas'].target='_self';
		document.forms['areas'].action='<?= $PHP_SELF ?>';
		document.forms['areas'].relatorio.value='especifica��o_tecnica_subsistema';
		document.forms['areas'].acao.value='salvar';
		document.forms['areas'].submit();
...
//	COMENTADO				$sql_dispositivos .= "GROUP BY dispositivo, ds_dispositivo ";	
							...
							while ($regs_dispositivos = mysqli_fetch_array($reg_dispositivos))
								{
									?>
                  <option value="<?= $regs_dispositivos["ds_dispositivo"] ?>"<? if($regs_dispositivos["ds_dispositivo"]==$_POST["ds_dispositivo"]){echo 'selected';} ?>>
                  <?
/* COMENTADO				 
...
<?
	if($envio_rel)
	{
		...
		if($_POST["relatorio"]=='especifica��o_tecnica_area')
		{
			?>	
				<script>	
