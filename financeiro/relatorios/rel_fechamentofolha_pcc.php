...
{    
	...
	$this->Cell(30,5,"VALOR (4,65%)",0,0,'L',0);
...
$pdf->titulo="RELATÓRIO DE NOTAS FISCAIS - RETENÇÕES PIS, COFINS, CSLL (4,65%)";
...
{	
	...
	//Verifica se o tipo de Relatório selecionado é de PCC e se existiu imposto para esse fechamento.
	if($regs["valor_pcc"]!="0.00")
	{
		$pdf->Cell(30,5,"R$ " . formatavalor(number_format($valor_outros,2,".","")),0,0,'L',0);
	
		$vlr_total_outros += $valor_outros;		
	}
	else
	{
...
if($vlr_total_outros!=0)
{
	$pdf->HCell(21,5,"R$ " . formatavalor(number_format($vlr_total_outros,2,".","")),0,0,'L',0);
}
else
{
...
$pdf->Output('FECHAMENTO_PCC_'.date('dmYHmi').'.pdf','D');

//Grava o arquivo PDF em uma pasta, no formato "TIPO AnoMesInicial-AnoMesFinal DataGeracao.pdf".
$pdf->Output(DOCUMENTOS_FINANCEIRO.COMPROVANTES_FECHAMENTO. "PCC_" . str_replace("-","",$datas[0]) . "-" . str_replace("-","",$datas[1]) . " " . date("dmY") . '.pdf','F');
