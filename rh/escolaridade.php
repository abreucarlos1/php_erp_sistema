...
function voltar()
{
	...
	$resposta -> addScriptCall("reset_campos('frm_escolaridade')");
	
	$resposta -> addAssign("btninserir", "value", "Inserir");
	
	$resposta -> addEvent("btninserir", "onclick", "xajax_insere(xajax.getFormValues('frm_escolaridade'));");
...
function atualizatabela($filtro)
{
	...
	{		
		...
		$sql_filtro = "AND rh_escolaridade.escolaridade LIKE '".$sql_texto."' ";
	...
	$resposta->addScript("grid('escolaridades', true, '460', '".$conteudo."');");
...
{	
	...
	if($dados_form["escolaridade"]!='')
	{
		$sql = "SELECT id_rh_escolaridade FROM ".DATABASE.".rh_escolaridade ";
		$sql .= "WHERE rh_escolaridade.escolaridade = '".maiusculas($dados_form["escolaridade"])."' ";
		$sql .= "AND reg_del = 0 ";
		...
		{
			$isql = "INSERT INTO ".DATABASE.".rh_escolaridade ";
			$isql .= "(escolaridade) ";
			$isql .= "VALUES ('" . maiusculas($dados_form["escolaridade"]) . "') ";
			...
			$resposta->addAlert("Escolaridade cadastrado com sucesso.");			
		}
		else
		{
...
function editar($id)
{
	...
	$sql = "SELECT * FROM ".DATABASE.".rh_escolaridade ";
	$sql .= "WHERE rh_escolaridade.id_rh_escolaridade = '".$id."' ";
	$sql .= "AND reg_del = 0 ";
	...
	$resposta -> addAssign("id_escolaridade", "value",$id);
	
	$resposta -> addAssign("escolaridade", "value",$regs["escolaridade"]);
	
	$resposta -> addAssign("btninserir", "value", "Atualizar");

	$resposta -> addEvent("btninserir", "onclick", "xajax_atualizar(xajax.getFormValues('frm_escolaridade'));");
...
function atualizar($dados_form)
{
	...
	{		
		...
		if($db->numero_registros==0)
		{
			$usql = "UPDATE ".DATABASE.".rh_escolaridade SET ";
			$usql .= "rh_escolaridade.escolaridade = '" . maiusculas($dados_form["escolaridade"]) . "' ";
			$usql .= "WHERE rh_escolaridade.id_rh_escolaridade = '".$dados_form["id_escolaridade"]."' ";
			...
			$resposta->addAlert("Escolaridade atualizada com sucesso.");
		}
		else
		{
			$resposta->addAlert("Registro já existente no banco de dados.");			
...
function excluir($id, $what)
{
	...
	$usql .= "WHERE rh_escolaridade.id_rh_escolaridade = '".$id."' ";
...
function grid(tabela, autoh, height, xml)
{
	...
	mygrid.setHeader("Escolaridade, D");
...
$smarty->assign('campo', $conf->campos('escolaridade'));

$smarty->assign('revisao_documento', 'V5');

$smarty->assign("classe",CSS_FILE);

$smarty->display('escolaridade.tpl');
?>