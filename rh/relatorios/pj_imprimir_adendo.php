...
/*
		...
		../rh/relatorios/pj_imprimir_adendo.php
...
$sql = "SELECT * FROM ".DATABASE.".setores, ".DATABASE.".local, ".DATABASE.".pj_contratos, ".DATABASE.".pj_tipo_contratacao, ".DATABASE.".pj_contratos_adendos ";
...
$sql .= "AND pj_contratos_adendos.id_contratos_adendos = '".$_GET["id_adendo"]."' ";
$sql .= "AND pj_contratos_adendos.id_contrato = pj_contratos.id_contrato ";
...
switch ($cont["id_tipo_adendo"])
{
	case 1: //prorrogacao prazo
		$document = $PHPWord->loadTemplate('../modelos_word/prorrogacao_prazo.docx');
		$nome_doc = "prorrogacao_prazo_".sprintf("%04d",$cont["id_contrato"])."/".substr($cont["data_inicio"],0,4).".docx";
	break;
	
	case 2: //reajuste
		$document = $PHPWord->loadTemplate('../modelos_word/prorrogacao_tarifa.docx');
		$nome_doc = "prorrogacao_tarifa_".sprintf("%04d",$cont["id_contrato"])."/".substr($cont["data_inicio"],0,4).".docx";
	break;
	
	case 3: //rescisão colaborador
		$document = $PHPWord->loadTemplate('../modelos_word/rescisao_colaborador.docx');
		$nome_doc = "rescisao_colaborador_".sprintf("%04d",$cont["id_contrato"])."/".substr($cont["data_inicio"],0,4).".docx";
	break;
	
	case 4: //rescisão empresa
		$document = $PHPWord->loadTemplate('../modelos_word/rescisao_empresa.docx');
		$nome_doc = "rescisao_empresa_".sprintf("%04d",$cont["id_contrato"])."/".substr($cont["data_inicio"],0,4).".docx";
	break;
	
	case 5: //distrato
		$document = $PHPWord->loadTemplate('../modelos_word/distrato.docx');
		$nome_doc = "distrato_".sprintf("%04d",$cont["id_contrato"])."/".substr($cont["data_inicio"],0,4).".docx";
	break;
}
...
{
	$document->setValue('Value22', formatavalor($cont["valor"])." por hora");
}
else
{
	$document->setValue('Value22', formatavalor($cont["valor"])." por mes");
}

$document->setValue('Value23', mysql_php($cont["data_prorrogacao_ini"]));
$document->setValue('Value24', mysql_php($cont["data_prorrogacao_fim"]));
$document->setValue('Value25', substr($cont["data_prorrogacao_ini"],8,2). " de ".meses(((int)substr($cont["data_prorrogacao_ini"],5,2)-1),1)." de ".substr($cont["data_prorrogacao_ini"],0,4));
$document->setValue('Value26', mysql_php($cont["data_final"]));

$document->setValue('Value27', substr($cont["data_final"],8,2). " de ".meses(((int)substr($cont["data_final"],5,2)-1),1)." de ".substr($cont["data_final"],0,4));
...
header("Content-Disposition: attachment; filename=".$nome_doc);
