...
function inserir($dados_form)
{
	...
	$isql = "INSERT INTO
				".DATABASE.".grupo_x_sub_grupo (id_grupo, id_sub_grupo)
			VALUES 
...
function atualizatabela($dados_form)
{
	...
	SELECT
	  *
	  FROM
	    ".DATABASE.".grupo_x_sub_grupo a
	     JOIN(
	      SELECT id_grupo codGrupo, grupo, codigo_grupo FROM ".DATABASE.".grupo WHERE reg_del = 0 AND codigo_grupo = ".$codigo_grupo[0]." 
	    ) grupo
...
function editar($codigoInteligente)
{
	...
	$sql = "SELECT * FROM ".DATABASE.".componentes WHERE componentes.reg_del = 0 AND componentes.codigo_inteligente = '".$codigoInteligente."'";	
...
function excluir($id)
{
	...
	/*
	$dsql = "DELETE FROM ".DATABASE.".grupo_x_sub_grupo WHERE id_grupo_x_sub_grupo = ".$id;
	
	$db->delete($dsql, 'MYSQL');
	*/

	$usql = "UPDATE ".DATABASE.".grupo_x_sub_grupo SET ";
	$usql .= "reg_del = 1, ";
